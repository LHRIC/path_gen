...
FILENAME = "ref_gates_enduranceTwist.csv"
FILENAME = "ref_less_gates_twist.csv"
...
USE_DOF_GUESS = True
GUESS_DOF_FILE = "out_less_gates_twist_windowed_test1/opt_dofs_2026-08-23_01-07-13.npy"

# Objective Function Integration
OBJ_FUN_INTEGRATION_EPSABS = 1.0e-2 #I use 1.0 e-3 for small tracks, e-2 for autoX and endurance tho
OBJ_FUN_INTEGRATION_EPSREL = 1.0e-6 #default, haven't fucked with
...
#DOF Guesses
# Build the geometric initial-guess DOF array for a set of gates. Returns the
# flat array generate_spline expects: [gate position, D1x/D1y, D2x/D2y].
#   - gate position guess: 0.5 (cross each gate at its midpoint)
#   - D1 guess: parallel to the average of the previous and next gate directions
#   - D2 guess: zero
def initial_guess(gate_points):
...
guess_dofs = initial_guess(imported_gate_points)

# Optional warm-start: replace the geometric guess with DOFs loaded from a file.
# (Inlined instead of importing unpack_dofs.load_dof_guess -- unpack_dofs itself
# imports this module, so importing it back from here while running as __main__
# is a circular import.)
if USE_DOF_GUESS:
    guess_dofs = np.load(GUESS_DOF_FILE)
    if len(guess_dofs) != gate_ct * 5:
        raise ValueError(
            f"Guess DOF file '{GUESS_DOF_FILE}' has {len(guess_dofs)} values, "
            f"expected {gate_ct * 5} for {gate_ct} gates.")
    print(f"Loaded guess DOFs from: {GUESS_DOF_FILE} ({len(guess_dofs)} values)")
...
def save_iteration(current_dofs):
    ...
    t = np.linspace(0, gate_ct-1, OBJ_PLOT_POINTS_PER_GATE*gate_ct)
...
def plot_spline_debug():
    ...
    t = np.linspace(0,times[-1],PATHGEN_POINT_COUNT)  # coarse: discrete path-gen dots
    t_smooth = np.linspace(0,times[-1],OBJ_PLOT_POINTS_PER_GATE*gate_ct)  # dense: smooth lines
    x_path_pts, y_path_pts = generate_path_t(x_spline, y_spline, t)
    x_path_smooth, y_path_smooth = generate_path_t(x_spline, y_spline, t_smooth)
    fig, ax = plt.subplots()
    ax.scatter(gate_x_pts,gate_y_pts)
    ax.plot(x_path_smooth,y_path_smooth)
    ...
    bx.plot(t_smooth,x_spline(t_smooth))
    bx.plot(t_smooth,y_spline(t_smooth))
