...
guess_d1_dofs = np.ones(gate_ct*2)
guess_d2_dofs = np.ones(gate_ct*2)*10
...
d1_dof_lb = [-10]*gate_ct*2
d1_dof_ub = [10]*gate_ct*2
d2_dof_lb = [-1]*gate_ct*2
d2_dof_ub = [1]*gate_ct*2
...
res = minimize(raw_obj_fun, guess_dofs, method='SLSQP', bounds = dof_bounds,
               options={'maxiter': 20, 'ftol': 1e-8, 'disp': True})
print(res)
...
equidistant_times = np.interp(np.linspace(0, total_length, 500), distance_t_full, t_uniform)
